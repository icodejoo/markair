...
.DESCRIPTION
    ...
    这里不单独编译一个只含 fuzz 用例的可执行文件——markair_tests.exe 本来就是
    唯一的测试目标（tests\CMakeLists.txt 的注释里明确"MARKAIR_TEST 的全局构造
    函数注册手法只允许存在于这里"，没有做用例过滤的命令行接口），所以直接
    跑整个 markair_tests.exe：
...
.PARAMETER ForceRebuild
    即使构建产物已存在，也强制重新构建一次 markair_tests 目标。

.PARAMETER TimeoutSeconds
    markair_tests.exe 整体运行超时秒数（防御性兜底：正常情况下全部语料
...
Write-Host "==== markair 畸形文档语料稳健性门禁（T43） ===="
...
if ($needBuild) {
    Write-Host "[1/2] 未找到 markair_tests.exe 或指定强制重建，开始构建（$BuildConfig）..."
    ...
    cmake --build $buildDir --target markair_tests --config $BuildConfig
    if ($LASTEXITCODE -ne 0) {
        Write-Host "FAIL: CMake 构建失败，退出码 $LASTEXITCODE"
        exit 1
    }
} else {
    Write-Host "[1/2] markair_tests.exe 已存在，跳过构建（传 -ForceRebuild 可强制重建）。"
}
...
Write-Host ""
Write-Host "[2/2] 运行 markair_tests.exe（含 test_fuzz_smoke.cpp 的畸形语料用例）..."

# 与 check_budget.ps1 同样的原因：PowerShell 5.1 下 $ErrorActionPreference
# = "Stop" 会把原生程序写 stderr 的行为包装成终止性 ErrorRecord，即便退出码
# 是 0（markair_tests 的逐用例耗时/截断状态汇总走的就是 stderr）。这里用后台
...
if (-not $finished) {
    Write-Host "FAIL: markair_tests.exe 超过 $TimeoutSeconds 秒未退出，判定为卡死，强制结束进程。"
    try { $proc.Kill() } catch {}
    exit 1
}
...
if ($exitCode -ne 0) {
    Write-Host "FAIL: markair_tests.exe 退出码为 $exitCode（应为 0）。"
    exit $exitCode
}
