...
.DESCRIPTION
    ...
    6. 高亮体积增量硬线门禁（T54，850000 字节，2026-09-19 因 SVG 支持上调、
       2026-09-22 因 CI 缓存 bug 期间实际体积失控再次上调，与上一条并存，
       见裁决 #8①）；
...
.PARAMETER HighlightExeSizeThresholdBytes
    ...
    门禁，单位字节。默认 850000（2026-09-22 用户裁决再次上调，理由见下）。
    ...
    2026-09-22 再次上调理由：CI 此前因为 check_budget.ps1 的"exe 已存在即
    跳过构建"缓存 bug（同日已修复，见第一步注释）长期没有真正重新构建，这
    条门禁事实上失效了好几个 commit；期间陆续落地的文件夹侧栏/历史侧栏/
    底部操作栏/文本选择/静默自动更新等功能把体积推到了 815104 字节，直到
    这次修复缓存 bug 才第一次被真实测出超标。用户裁决：不排查具体是哪块
    功能贡献了这部分增量，直接把阈值上调到 850000（留约 35KB 余量），把
    "重新对齐真实体积基线"这件事本身当成本次修复的一部分，不阻塞发布。
...
param(
    ...
    [double]$HighlightExeSizeThresholdBytes = 850000,
