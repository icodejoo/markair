// mdvn "打开文档"按钮(新需求):弹出 Windows 标准文件选择对话框
// (`IFileOpenDialog`,不用老的 `GetOpenFileNameW`),选中文件后用
// `CreateProcessW` 新开一个独立的 mdvn.exe 进程打开它——不复用现有的
...
//
// 用 `CreateProcessW` 新开的这个进程是用户主动发起的一个新的独立 mdvn 实例,
// 不是当前 mdvn.exe 派生的"子进程"链——01§4"单个文档实例子进程数恒为 0"
// 约束的是"打开/切换文档过程中 mdvn 自身不得派生子进程",不禁止用户主动
// 触发"新开一个 mdvn 窗口"这个动作本身(与双击资源管理器里另一个 .md 文件
 ...
 *   if (mdvn::ShowOpenMarkdownDialog(hwnd, path, MAX_PATH)) { ... }
 */
 ...
 * @example
 *   wchar_t buf[64];
 *   mdvn::BuildLaunchCommandLine(L"C:\\mdvn.exe", L"C:\\a b.md", buf, 64);
 *   // buf == L"\"C:\\mdvn.exe\" \"C:\\a b.md\""
 */
bool BuildLaunchCommandLine(const wchar_t* exePath, const wchar_t* filePath,
                             wchar_t* outCmdLine, u32 outCap);

/**
 * 用 `CreateProcessW` 拉起一个新的 mdvn.exe 进程打开 `filePath`(命令行参数
 ...
 * @example mdvn::LaunchNewInstance(L"C:\\docs\\readme.md");
 */
