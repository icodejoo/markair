...
## 待办(2026-09-18 用户提出,记录暂不实现)

**新功能需求**:窗口底部加一个底部操作栏,5个功能按钮(字体放大 / 字体缩小 / 主题切换 / 打开文档 / 打开大纲),点击执行对应动作。

**为什么现在不做**:M3(`08-m3-tasks.md`)的边界明文写着"M3 是打磨与发布阶段,不新增任何用户可见功能"。这是一条新增可见功能(常驻UI元素),与 M3 范围直接冲突。已告知用户,暂记为待办,等 M3 收尾后再确认排期(单独开 M3.5/M4,还是插队)。

**注意**:落地时需要考虑的现有约束——按钮涉及的4个动作(字体缩放/主题切换/打开文档/大纲)在现有代码里都已有键盘快捷键入口(`Ctrl+±`、`Ctrl+Shift+T`、命令行/双击、`Ctrl+\`),UI栏本质是给这些已有能力加一层鼠标可点的入口,不是从零实现功能;但"常驻底部栏"本身是一块新增的常驻UI状态/绘制对象,与 M2 裁决 #7"大纲侧栏默认关闭、开销为0"等既有"能省则省"的设计取向需要协调(常驻栏默认可见的话,内存/首帧预算要重新核算)。

**状态更新(2026-09-19):M3全部完成后进入实现阶段。**
