...
using mdvn::History;
using mdvn::HistoryEntry;
using mdvn::kHistoryCapacity;
...
MDVN_TEST(History_PopBackOnEmptyStackIsNoOp) {
    History history;
    History before = history;
    HistoryEntry entry;
    bool popped = history.PopBack(&entry);
    MDVN_CHECK(!popped);
    MDVN_CHECK(SnapshotsEqual(history, before));
}

// 2. 空栈按 Alt+→ 同理无动作。
MDVN_TEST(History_PopForwardOnEmptyStackIsNoOp) {
    History history;
    History before = history;
    HistoryEntry entry;
...
MDVN_TEST(History_Push33rdEntryEvictsOldest) {
    History history;
    for (mdvn::u32 i = 0; i < kHistoryCapacity; ++i) {
        wchar_t path[16];
        swprintf_s(path, L"C:\\%u.md", i);
        history.PushNavigation(path, static_cast<float>(i));
    }
    MDVN_CHECK_EQ(history.BackCount(), kHistoryCapacity);

    // 第 33 条压入:此时栈底应是第 0 条("C:\0.md"),压入后应被挤掉。
    history.PushNavigation(L"C:\\33.md", 33.0f);
    MDVN_CHECK_EQ(history.BackCount(), kHistoryCapacity);  // 仍然是上限,没有变多

    // 从栈顶往下弹出全部 32 条,顺序应是 33,31,30,...,1(第 0 条已被挤掉)。
    HistoryEntry e;
    MDVN_CHECK(history.PopBack(&e));
    MDVN_CHECK(wcscmp(e.path, L"C:\\33.md") == 0);
    for (mdvn::u32 i = 0; i < kHistoryCapacity - 1; ++i) {
        MDVN_CHECK(history.PopBack(&e));
    }
    MDVN_CHECK_EQ(history.BackCount(), 0u);
    ...
    HistoryEntry last{};
    for (mdvn::u32 i = 0; i < kHistoryCapacity; ++i) {
        MDVN_CHECK(history2.PopBack(&last));
    }
    MDVN_CHECK(wcscmp(last.path, L"C:\\1.md") == 0);  // 编号 0 已被挤掉,最旧的是 1
}

// 4. 前进栈在新导航(PushNavigation)时被清空。
MDVN_TEST(History_PushNavigationClearsForwardStack) {
    History history;
    history.PushNavigation(L"C:\\a.md", 0.0f);
    history.PushForwardRaw(L"C:\\b.md", 10.0f);  // 模拟一次成功的后退,填了前进栈
    MDVN_CHECK_EQ(history.ForwardCount(), 1u);

    history.PushNavigation(L"C:\\c.md", 20.0f);  // 新导航
    MDVN_CHECK_EQ(history.ForwardCount(), 0u);
}

// 5. 同路径、不同 scrollY 的两条记录(T36③锚点跳转的记录方式)。
MDVN_TEST(History_SamePathDifferentScrollYTwoEntries) {
    History history;
    history.PushNavigation(L"C:\\a.md", 0.0f);
    history.PushNavigation(L"C:\\a.md", 200.0f);
    MDVN_CHECK_EQ(history.BackCount(), 2u);

    HistoryEntry top;
    MDVN_CHECK(history.PopBack(&top));
    MDVN_CHECK(wcscmp(top.path, L"C:\\a.md") == 0);
    MDVN_CHECK(top.scrollY == 200.0f);

    HistoryEntry bottom;
    MDVN_CHECK(history.PopBack(&bottom));
    MDVN_CHECK(wcscmp(bottom.path, L"C:\\a.md") == 0);
    MDVN_CHECK(bottom.scrollY == 0.0f);
}

// 6. PushBackRaw 不清空前进栈(与 PushNavigation 的关键区别,前进导航用)。
MDVN_TEST(History_PushBackRawDoesNotClearForwardStack) {
    History history;
    history.PushNavigation(L"C:\\a.md", 0.0f);
    ...
    MDVN_CHECK_EQ(history.ForwardCount(), 1u);  // 前进栈不受影响
    MDVN_CHECK_EQ(history.BackCount(), 2u);
}

// 7. PushForwardRaw 不清空后退栈。
MDVN_TEST(History_PushForwardRawDoesNotClearBackStack) {
    History history;
    history.PushNavigation(L"C:\\a.md", 0.0f);
    ...
    MDVN_CHECK_EQ(history.BackCount(), 2u);  // 后退栈不受影响
    MDVN_CHECK_EQ(history.ForwardCount(), 1u);
}

// 8. PopBack 弹出的记录会真正从栈里移除("路径不存在的剔除"就是靠这一点
// 天然满足——调用方发现文件不存在时什么都不用做,不需要额外的剔除步骤)。
MDVN_TEST(History_PopBackRemovesEntryFromStack) {
    History history;
    history.PushNavigation(L"C:\\deleted.md", 0.0f);
    MDVN_CHECK_EQ(history.BackCount(), 1u);

    HistoryEntry entry;
    MDVN_CHECK(history.PopBack(&entry));
    MDVN_CHECK_EQ(history.BackCount(), 0u);  // 已经被摘掉,不会再弹出第二次
    HistoryEntry again;
    MDVN_CHECK(!history.PopBack(&again));
}
...
MDVN_TEST(History_NonNavigationReloadDoesNotTouchStack) {
    History history;
    history.PushNavigation(L"C:\\a.md", 0.0f);
    ...
    MDVN_CHECK(SnapshotsEqual(history, before));
    MDVN_CHECK_EQ(history.BackCount(), before.BackCount());
    MDVN_CHECK_EQ(history.ForwardCount(), before.ForwardCount());
}

// 10. 路径超长时会被截断,不会越界写(kHistoryPathCapacity 的边界情况)。
MDVN_TEST(History_OverlongPathIsTruncatedNotOverrun) {
    History history;
    ...
    MDVN_CHECK(len < mdvn::kHistoryPathCapacity);
    MDVN_CHECK(entry.path[len] == L'\0');
}

// 11. nullptr 路径不崩溃,写成空字符串。
MDVN_TEST(History_NullPathBecomesEmptyString) {
    History history;
    history.PushNavigation(nullptr, 1.0f);
    HistoryEntry entry;
    MDVN_CHECK(history.PopBack(&entry));
    MDVN_CHECK(wcscmp(entry.path, L"") == 0);
}

// 12. 后退/前进两个栈各自独立计数,压其中一个不影响另一个的计数基线。
MDVN_TEST(History_BackAndForwardCountsAreIndependent) {
    History history;
    MDVN_CHECK_EQ(history.BackCount(), 0u);
    MDVN_CHECK_EQ(history.ForwardCount(), 0u);
...
MDVN_TEST(History_ExactlyFullDoesNotEvict) {
    History history;
    for (mdvn::u32 i = 0; i < kHistoryCapacity; ++i) {
        wchar_t path[16];
        swprintf_s(path, L"C:\\%u.md", i);
    ...
    HistoryEntry last{};
    for (mdvn::u32 i = 0; i < kHistoryCapacity; ++i) {
        MDVN_CHECK(history.PopBack(&last));
    }
    MDVN_CHECK(wcscmp(last.path, L"C:\\0.md") == 0);  // 恰好 32 条时编号 0 没被挤掉
}

// sizeof(HistoryEntry) 在预期范围内(约 520B:260 个 wchar_t + 1 个 float,
// 按 4 字节对齐补到 524~528B),用来佐证任务描述里"约 520B × 32 ≈ 17KB"的
// 内存估算不是空口白话。用 static_assert 而不是 MDVN_CHECK——sizeof 是编译期
