...
StrSlice Src(const char* s) { return StrSlice{s, static_cast<markair::u32>(strlen(s))}; }

} // namespace

// 用例:没有任何标题的文档,提取结果应为 0 条。
MARKAIR_TEST(Outline_NoHeadings_ExtractsNothing) {
    Arena arena;
    MARKAIR_CHECK(arena.Init(4 * 1024 * 1024));
    Document doc = ParseMarkdown(Src("just a paragraph\n\nanother one\n"), &arena);

    Vec<OutlineItem> items(&arena);
    u32 n = ExtractOutline(doc, &items);
    MARKAIR_CHECK_EQ(n, 0u);
    MARKAIR_CHECK_EQ(items.Size(), 0u);
}

// 用例:只有若干个 H1,层级全为 1,块下标升序。
MARKAIR_TEST(Outline_OnlyH1_ExtractsAllWithLevelOne) {
    Arena arena;
    MARKAIR_CHECK(arena.Init(4 * 1024 * 1024));
    ...
    MARKAIR_CHECK_EQ(items.Size(), 3u);
    for (markair::u32 i = 0; i < items.Size(); ++i) {
        MARKAIR_CHECK_EQ(static_cast<int>(items[i].level), 1);
        if (i > 0) MARKAIR_CHECK(items[i].blockIdx > items[i - 1].blockIdx);
    }
}

// 用例:跳级(H1 直接到 H4,中间没有 H2/H3),大纲应照样按原始层级记录,
// 不做任何"补齐"或纠正——这是数据提取层,不负责展示逻辑。
MARKAIR_TEST(Outline_SkippedLevels_RecordsRawLevels) {
    Arena arena;
    MARKAIR_CHECK(arena.Init(4 * 1024 * 1024));
    ...
    Vec<OutlineItem> items(&arena);
    u32 n = ExtractOutline(doc, &items);
    MARKAIR_CHECK_EQ(n, 2u);
    MARKAIR_CHECK_EQ(static_cast<int>(items[0].level), 1);
    MARKAIR_CHECK_EQ(static_cast<int>(items[1].level), 4);
}

// 用例:300 条标题(循环 1-6 级),提取条数与层级都要精确匹配。
MARKAIR_TEST(Outline_ThreeHundredHeadings_ExtractsAllInOrder) {
    Arena arena;
    MARKAIR_CHECK(arena.Init(16 * 1024 * 1024));
    ...
    Document doc = ParseMarkdown(StrSlice{buf, static_cast<markair::u32>(pos)}, &arena);

    Vec<OutlineItem> items(&arena);
    u32 n = ExtractOutline(doc, &items);
    MARKAIR_CHECK_EQ(n, 300u);
    MARKAIR_CHECK_EQ(items.Size(), 300u);
    for (int i = 0; i < 300; ++i) {
        int expectLevel = (i % 6) + 1;
        MARKAIR_CHECK_EQ(static_cast<int>(items[static_cast<markair::u32>(i)].level), expectLevel);
        if (i > 0) {
            MARKAIR_CHECK(items[static_cast<markair::u32>(i)].blockIdx >
                       items[static_cast<markair::u32>(i - 1)].blockIdx);
        }
    }
}
...
MARKAIR_TEST(Outline_HeadingInsideBlockQuote_StillExtracted) {
    Arena arena;
    MARKAIR_CHECK(arena.Init(4 * 1024 * 1024));
    ...
    MARKAIR_CHECK_EQ(static_cast<int>(items[0].level), 1);
    MARKAIR_CHECK_EQ(doc.blocks[items[0].blockIdx].type, markair::BlockType::Heading);
}
...
MARKAIR_TEST(Outline_BenchA_ExtractionUnderHalfMillisecond) {
    wchar_t path[600];
    swprintf_s(path, 600, L"%hs\\BENCH-A.md", MARKAIR_BENCH_DIR);
    ...
    Arena arena;
    MARKAIR_CHECK(arena.Init(64 * 1024 * 1024));
