...
using markair::ApplyCascadeOffset;
using markair::ClampMinimumSize;
using markair::ClampWindowRectToMonitors;
using markair::FindMonitorContaining;
using markair::kCascadeOffsetDip;
using markair::i32;
using markair::kMinWindowHeight;
using markair::kMinWindowWidth;
using markair::MonitorRect;
using markair::RectI;
using markair::RectsOverlap;
using markair::SameOrigin;
using markair::u32;
...
MARKAIR_TEST(WindowState_SingleMonitorRestoresUnchanged) {
    MonitorRect mon = SinglePrimaryMonitor();
    RectI saved{100, 100, 900, 700};
    RectI result = ClampWindowRectToMonitors(saved, &mon, 1, 0);
    MARKAIR_CHECK(RectEquals(result, saved));
}

// 用例:副屏被拔掉 —— 保存时的矩形落在原来的副屏(已不存在)上,
// 恢复时只有主屏这一个显示器,越界必须钳到主屏工作区中心。
MARKAIR_TEST(WindowState_SecondaryMonitorRemovedClampsToPrimaryCenter) {
    MonitorRect mon = SinglePrimaryMonitor();
    RectI saved{2000, 100, 2800, 700};  // 原本在副屏(1920 起)上,现在副屏不存在
    RectI result = ClampWindowRectToMonitors(saved, &mon, 1, 0);

    MARKAIR_CHECK(FindMonitorContaining(result, &mon, 1) == 0);
    // 居中:工作区宽 1920、高 1040,窗口宽 800、高 600。
    i32 expectedLeft = 0 + (1920 - 800) / 2;
    i32 expectedTop = 0 + (1040 - 600) / 2;
    MARKAIR_CHECK_EQ(result.left, expectedLeft);
    MARKAIR_CHECK_EQ(result.top, expectedTop);
    MARKAIR_CHECK_EQ(result.Width(), 800);
    MARKAIR_CHECK_EQ(result.Height(), 600);
}

// 用例:副屏在主屏左侧(负坐标)—— 合法情形,不能被当成越界。
MARKAIR_TEST(WindowState_NegativeCoordinatesOnLeftMonitorAreLegal) {
    MonitorRect mons[] = {
    ...
    MARKAIR_CHECK(RectEquals(result, saved));  // 负坐标本身合法,原样保留
    MARKAIR_CHECK(FindMonitorContaining(result, mons, 2) == 1);
}

// 用例:矩形部分越界 —— 只要与某个显示器还有重叠(标题栏可见)就算合法,
// 不触发居中钳制。
MARKAIR_TEST(WindowState_PartiallyOffscreenIsAllowed) {
    MonitorRect mon = SinglePrimaryMonitor();
    RectI saved{1800, 100, 2600, 700};  // 右边界超出 1920,但左边界还在屏内
    RectI result = ClampWindowRectToMonitors(saved, &mon, 1, 0);
    MARKAIR_CHECK(RectEquals(result, saved));  // 部分越界不钳制位置
    MARKAIR_CHECK(FindMonitorContaining(result, &mon, 1) == 0);
}

// 用例:矩形完全越界 —— 与任何显示器都没有重叠,必须钳到主屏工作区中心。
MARKAIR_TEST(WindowState_FullyOffscreenClampsToPrimaryCenter) {
    MonitorRect mon = SinglePrimaryMonitor();
    RectI saved{5000, 5000, 5800, 5600};  // 与唯一显示器毫无重叠
    RectI result = ClampWindowRectToMonitors(saved, &mon, 1, 0);
    MARKAIR_CHECK(FindMonitorContaining(result, &mon, 1) == 0);
    MARKAIR_CHECK_EQ(result.left, (1920 - 800) / 2);
    MARKAIR_CHECK_EQ(result.top, (1040 - 600) / 2);
}

// 用例:尺寸小于下限 —— 无论是否越界,尺寸都要先被钳到 kMinWindowWidth/Height。
MARKAIR_TEST(WindowState_TinySizeClampedToMinimum) {
    MonitorRect mon = SinglePrimaryMonitor();
    RectI tiny{100, 100, 101, 101};  // 1x1
    RectI result = ClampWindowRectToMonitors(tiny, &mon, 1, 0);
    MARKAIR_CHECK_EQ(result.Width(), kMinWindowWidth);
    MARKAIR_CHECK_EQ(result.Height(), kMinWindowHeight);
    MARKAIR_CHECK_EQ(result.left, 100);  // 原点不动,只扩右/下边界
    MARKAIR_CHECK_EQ(result.top, 100);
    ...
    MARKAIR_CHECK_EQ(sized.Width(), kMinWindowWidth);
    MARKAIR_CHECK_EQ(sized.Height(), kMinWindowHeight);
}
...
MARKAIR_TEST(WindowState_MaximizedRestoreUsesNormalRectClamping) {
    MonitorRect mon = SinglePrimaryMonitor();
...
// 用例:无同位窗口时不偏移。
MARKAIR_TEST(WindowState_CascadeNoOffsetWithoutExistingWindow) {
    RectI base{100, 100, 900, 700};
    RectI workArea{0, 0, 1920, 1040};
    RectI result = ApplyCascadeOffset(base, nullptr, 0, workArea, 24);
    MARKAIR_CHECK(RectEquals(result, base));
}

// 用例:有 1 个同位窗口时偏移 24px(单测用像素直接验证,真实调用方会把
// kCascadeOffsetDip 按 DPI 换算成物理像素再传入)。
MARKAIR_TEST(WindowState_CascadeOffsetsBy24WhenOneWindowAtSamePosition) {
    RectI base{100, 100, 900, 700};
    RectI existing[] = {base};  // 同一个原点已经有一个窗口
    RectI workArea{0, 0, 1920, 1040};
    RectI result = ApplyCascadeOffset(base, existing, 1, workArea,
                                       static_cast<markair::i32>(kCascadeOffsetDip));
    MARKAIR_CHECK_EQ(result.left, base.left + 24);
    MARKAIR_CHECK_EQ(result.top, base.top + 24);
    MARKAIR_CHECK_EQ(result.Width(), base.Width());  // 尺寸不变,只偏移原点
    MARKAIR_CHECK_EQ(result.Height(), base.Height());
    MARKAIR_CHECK(!SameOrigin(result, base));
}

// 用例:连续层叠到边界时回卷到起始位置。
MARKAIR_TEST(WindowState_CascadeWrapsAroundAtWorkAreaBoundary) {
    ...
    MARKAIR_CHECK(result2.right <= workArea.right || RectEquals(result2, base));
    MARKAIR_CHECK(result2.bottom <= workArea.bottom || RectEquals(result2, base));
}

// 用例:RectsOverlap / FindMonitorContaining 的基本边界行为(边缘相接不算重叠)。
MARKAIR_TEST(WindowState_RectsOverlapEdgeTouchingIsNotOverlap) {
    RectI a{0, 0, 100, 100};
    RectI b{100, 0, 200, 100};  // 恰好在 a 的右边缘相接,不重叠
    MARKAIR_CHECK(!RectsOverlap(a, b));

    RectI c{99, 0, 200, 100};  // 有 1px 真实重叠
    MARKAIR_CHECK(RectsOverlap(a, c));
}
