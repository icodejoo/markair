...
using markair::ComputeTableColumnWidths;
using markair::Document;
using markair::ParseMarkdown;
using markair::Span;
using markair::StrSlice;
using markair::Utf8VisualWidth;
using markair::kMinColumnWidthDip;
using markair::kMaxColumnWidthRatio;
using markair::kTableAvgCharWidthDip;
using markair::kTableCellPaddingDip;
using markair::kTableGlyphWidthSafetyFactor;
using markair::u32;
...
MARKAIR_TEST(Table_SingleColumnVeryLongContentClampedToRatio) {
    MARKAIR_MAKE_TEST_ARENA();
    u32 chars[] = {500}; // 1 列 1 行,极长内容
    Span<float> widths = ComputeTableColumnWidths(chars, 1, 1, 600.0f, &arena);
    MARKAIR_CHECK(widths.data != nullptr);
    MARKAIR_CHECK_EQ(widths.len, 1u);
    MARKAIR_CHECK(widths[0] <= 600.0f * kMaxColumnWidthRatio + 0.01f);
}

// 用例 2:空表头(字符数全 0)——理想宽度钳制到下限,不产生 0 宽列。
MARKAIR_TEST(Table_EmptyHeaderFallsBackToMinWidth) {
    MARKAIR_MAKE_TEST_ARENA();
    u32 chars[] = {0, 0, 0, 0}; // 2 列 2 行,全空
    Span<float> widths = ComputeTableColumnWidths(chars, 2, 2, 600.0f, &arena);
    MARKAIR_CHECK_EQ(widths.len, 2u);
    MARKAIR_CHECK_EQ(widths[0], kMinColumnWidthDip);
    MARKAIR_CHECK_EQ(widths[1], kMinColumnWidthDip);
}

// 用例 3:列数不齐(某些行缺数据按 0 补齐)——仍能正确取每列最大值。
MARKAIR_TEST(Table_RaggedRowsStillTakePerColumnMax) {
    MARKAIR_MAKE_TEST_ARENA();
    ...
    MARKAIR_CHECK_EQ(widths.len, 3u);
    // 第 0 列最大字符数是 9,理应比第 1/2 列(最大 5/3)更宽。
    MARKAIR_CHECK(widths[0] > widths[1]);
    MARKAIR_CHECK(widths[1] > widths[2]);
}

// 用例 4:100 列宽表——总宽远超视口时整体等比压缩,不出现小于下限的列,
// 且压缩后各列仍保持原有的相对比例顺序。
MARKAIR_TEST(Table_HundredColumnsCompressProportionally) {
    Arena bigArena;
    MARKAIR_CHECK(bigArena.Init(1 * 1024 * 1024));

    const u32 kCols = 100;
    u32 chars[kCols];
    for (u32 c = 0; c < kCols; ++c) chars[c] = 20; // 每列理想宽度相同
    Span<float> widths = ComputeTableColumnWidths(chars, kCols, 1, 800.0f, &bigArena);
    MARKAIR_CHECK_EQ(widths.len, kCols);

    for (u32 c = 0; c < kCols; ++c) {
        MARKAIR_CHECK(widths[c] >= kMinColumnWidthDip - 0.01f);
    }
    // 100 列同宽,压缩后应仍然彼此相等(等比压缩不改变相对比例)。
    for (u32 c = 1; c < kCols; ++c) {
        MARKAIR_CHECK(widths[c] == widths[0]);
    }
}

// 用例 5:总宽不超视口时不触发压缩,列宽就是各自的理想宽度(钳制到下限之后)。
MARKAIR_TEST(Table_NoCompressionWhenFitsViewport) {
    MARKAIR_MAKE_TEST_ARENA();
    ...
    MARKAIR_CHECK_EQ(widths[0], kMinColumnWidthDip);
    MARKAIR_CHECK(widths[1] > widths[0]);
}
...
MARKAIR_TEST(Table_LongColumnNotClampedWhenTableFitsViewport) {
    MARKAIR_MAKE_TEST_ARENA();
...
// 是硬约束,不是"尽量占满视口"的目标)。
MARKAIR_TEST(Table_CappedColumnDuringCompressionIsNotStretchedBack) {
    MARKAIR_MAKE_TEST_ARENA();
    u32 chars[] = {500}; // 与用例 1 相同,理想宽度远超视口
    Span<float> widths = ComputeTableColumnWidths(chars, 1, 1, 600.0f, &arena);
    MARKAIR_CHECK_EQ(widths.len, 1u);
    MARKAIR_CHECK_EQ(widths[0], 600.0f * kMaxColumnWidthRatio);
}

// 用例 6:colCount 为 0 时安全返回空 Span,不崩溃。
MARKAIR_TEST(Table_ZeroColumnsReturnsEmptySpan) {
    MARKAIR_MAKE_TEST_ARENA();
    Span<float> widths = ComputeTableColumnWidths(nullptr, 0, 0, 600.0f, &arena);
    MARKAIR_CHECK(widths.data == nullptr);
    MARKAIR_CHECK_EQ(widths.len, 0u);
}

// ---- Utf8VisualWidth:纯函数,验证宽字符/窄字符/混排的视觉宽度计数 ----

// 用例 7:纯 ASCII——每个字节 1 个视觉宽度单位,与字符数相等。
MARKAIR_TEST(VisualWidth_AsciiCountsOnePerChar) {
    MARKAIR_CHECK_EQ(Utf8VisualWidth(StrSlice{"Gamma", 5}), 5u);
}

// 用例 8:纯 CJK——"网关服务" 4 个汉字,每个记 2 个单位,共 8;
// UTF-8 下这 4 个汉字占 12 字节,验证结果是视觉宽度而不是字节数。
MARKAIR_TEST(VisualWidth_CjkCountsTwoPerChar) {
    const char text[] = "\xe7\xbd\x91\xe5\x85\xb3\xe6\x9c\x8d\xe5\x8a\xa1"; // 网关服务
    MARKAIR_CHECK_EQ(Utf8VisualWidth(StrSlice{text, sizeof(text) - 1}), 8u);
}

// 用例 9:中英混排——"a你b" = 1(a) + 2(你) + 1(b) = 4,不是字节数 5,也不是字符数 3。
MARKAIR_TEST(VisualWidth_MixedCjkAndAsciiSumsPerCharWidth) {
    const char text[] = "a\xe4\xbd\xa0" "b"; // a 你 b
    MARKAIR_CHECK_EQ(Utf8VisualWidth(StrSlice{text, sizeof(text) - 1}), 4u);
}

// 用例 10:空串视觉宽度为 0。
MARKAIR_TEST(VisualWidth_EmptyStringIsZero) {
    MARKAIR_CHECK_EQ(Utf8VisualWidth(StrSlice{"", 0}), 0u);
}

// 用例 11:韩文谚文音节区(2026-09-17 扩充覆盖范围)——"안녕"(你好)2 个音节,
// 每个记 2 个单位,共 4;UTF-8 下占 6 字节,验证走的是谚文音节区分支而不是
// 退化成默认的 1 个单位。
MARKAIR_TEST(VisualWidth_HangulSyllableCountsTwoPerChar) {
...
MARKAIR_TEST(Table_CjkAndAsciiMixedColumnDoesNotSquashAsciiWidth) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] =
        "| Name | Desc |\n"
        "|------|------|\n"
    ...
    MARKAIR_CHECK_EQ(tg.tableColWidths.len, 2u);
    ...
    MARKAIR_CHECK(tg.tableColWidths[0] >= kMinColumnWidthDip);
    float ratio = tg.tableColWidths[1] / tg.tableColWidths[0];
    MARKAIR_CHECK(ratio < 4.0f);  // 明显小于字节数口径下会出现的比例
}

// 用例 12(症状 2 回归):单元格内容是能在估算列宽下一行放下的中文短语时,
// 行高不应该被多算成 2 行——用视觉宽度而不是字节数估算折行,配合按视觉
// 宽度分配的列宽,两者口径一致,单行内容就应该估算成 1 行的高度。
MARKAIR_TEST(Table_SingleLineCjkCellDoesNotOverestimateRowHeight) {
    MARKAIR_MAKE_TEST_ARENA();
    ...
    MARKAIR_CHECK(bodyRow != markair::kInvalidIndex);
    ...
    // (允许极小的浮点误差)。
    MARKAIR_CHECK(headHeight > 0.0f);
    MARKAIR_CHECK(bodyHeight > 0.0f);
...
MARKAIR_TEST(Table_IdealWidthReservesGlyphSafetyMargin) {
    MARKAIR_MAKE_TEST_ARENA();
    u32 chars[] = {10};  // "1234567890":视觉宽度 10,真实排版约 86.2dip
    Span<float> widths = ComputeTableColumnWidths(chars, 1, 1, 2000.0f, &arena);
    MARKAIR_CHECK_EQ(widths.len, 1u);
    float textWidth = widths[0] - kTableCellPaddingDip * 2.0f;
    MARKAIR_CHECK(textWidth > 10.0f * kTableAvgCharWidthDip);
    MARKAIR_CHECK(textWidth >= 90.0f);  // 覆盖真实的 86.2dip,留有余量
}

// 用例 14(核心回归):字号缩放必须参与理想宽度计算。此前列宽恒按 1.0 档的
// 8dip/单位算,放大字号后真实字形同比变宽而列宽纹丝不动,于是无论把窗口拉多宽
// 表格都不变宽、单元格全部折行。
MARKAIR_TEST(Table_IdealWidthScalesWithFontScale) {
    MARKAIR_MAKE_TEST_ARENA();
    ...
    MARKAIR_CHECK_EQ(base.len, 2u);
    MARKAIR_CHECK_EQ(zoomed.len, 2u);
    ...
    MARKAIR_CHECK(diff < 0.01f);
    MARKAIR_CHECK(zoomed[1] > base[1]);
}

// 用例 15:fontScale 非法值(0 或负数)退化成 1.0,不产生 0 宽/负宽列。
MARKAIR_TEST(Table_NonPositiveFontScaleFallsBackToOne) {
    MARKAIR_MAKE_TEST_ARENA();
    u32 chars[] = {20};
    ...
    MARKAIR_CHECK_EQ(zero[0], normal[0]);
    MARKAIR_CHECK_EQ(negative[0], normal[0]);
}

// 用例 16(端到端回归):table_check.md 那张 7 列宽表,在足够宽的视口下
// 每一列的排版宽度都要放得下本列最长单元格的内容,且整表不被压缩——
// 这正是"窗口拉宽后表格应该一行展示更多内容"的验收点。
MARKAIR_TEST(Table_WideViewportFitsEveryCellOnOneLine) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] =
    ...
    BlockLayoutEngine layout;
    MARKAIR_CHECK(layout.Relayout(doc, 1600.0f));

    u32 tableIdx = FindBlockOfType(doc, BlockType::Table, 0);
    MARKAIR_CHECK(tableIdx != markair::kInvalidIndex);
    const BlockGeometry& tg = layout.Geometry(tableIdx);
    MARKAIR_CHECK_EQ(tg.tableColWidths.len, 7u);
    ...
    for (u32 i = 0; i < doc.blocks.Size(); ++i) {
        ...
        for (u32 k = 0; k < doc.blocks[i].inlineCount; ++k) {
            const markair::Inline& in = doc.inlines[doc.blocks[i].firstInlineIdx + k];
            visual += Utf8VisualWidth(StrSlice{markair::InlineTextBytes(in, doc), in.textLen});
        }
        float need = static_cast<float>(visual) * kTableAvgCharWidthDip * kTableGlyphWidthSafetyFactor;
        MARKAIR_CHECK(layout.Geometry(i).cellWidth + 0.01f >= need);
    }
    ...
    MARKAIR_CHECK(firstRowHeight > 0.0f);
}

// 用例 17:同一张表,视口越宽表格越宽——直到不再需要压缩为止;之后继续拉宽
// 不再变化(内容本身不多就不强行拉宽,见 table.h 算法注释第 2 步)。
MARKAIR_TEST(Table_WidensWithViewportUntilNoCompressionNeeded) {
    MARKAIR_MAKE_TEST_ARENA();
...
MARKAIR_TEST(Table_CompressionRespectsViewportWhenSomeColumnsHitFloor) {
    ...
    MARKAIR_CHECK_EQ(widths.len, 7u);
    ...
    MARKAIR_CHECK(total <= kViewport + 0.01f);
    // 且不能压过头:还有余地的那一列应当把剩下的空间吃满。
    MARKAIR_CHECK(total > kViewport - kMinColumnWidthDip);
}
