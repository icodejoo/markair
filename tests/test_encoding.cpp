...
using mdvn::DetectEncoding;
using mdvn::DecodeToUtf16;
...
MDVN_TEST(Encoding_Utf8NoBom_Chinese) {
    ...
    MDVN_CHECK(d.encoding == DetectedEncoding::Utf8NoBom);
    MDVN_CHECK_EQ(d.contentOffset, 0u);

    Utf16Slice w = DecodeToUtf16(s, d, &arena);
    MDVN_CHECK_EQ(w.len, 2u);
    MDVN_CHECK_EQ(static_cast<unsigned>(w.data[0]), 0x4F60u); // 你
    MDVN_CHECK_EQ(static_cast<unsigned>(w.data[1]), 0x597Du); // 好
}

// 样本 2:UTF-8 BOM(EF BB BF)+ "AB"。
MDVN_TEST(Encoding_Utf8Bom) {
    MDVN_MAKE_TEST_ARENA();
    const char bytes[] = {static_cast<char>(0xEF), static_cast<char>(0xBB), static_cast<char>(0xBF), 'A', 'B'};
    StrSlice s{bytes, sizeof(bytes)};

    EncodingDetection d = DetectEncoding(s);
    MDVN_CHECK(d.encoding == DetectedEncoding::Utf8Bom);
    MDVN_CHECK_EQ(d.contentOffset, 3u);

    Utf16Slice w = DecodeToUtf16(s, d, &arena);
    MDVN_CHECK_EQ(w.len, 2u);
    MDVN_CHECK_EQ(static_cast<int>(w.data[0]), 'A');
    MDVN_CHECK_EQ(static_cast<int>(w.data[1]), 'B');
}

// 样本 3:UTF-16LE BOM(FF FE)+ 'A'(0x41 0x00),另附一个奇数长度尾字节的边界情况。
MDVN_TEST(Encoding_Utf16LeBom) {
    MDVN_MAKE_TEST_ARENA();
    const char bytes[] = {static_cast<char>(0xFF), static_cast<char>(0xFE), 0x41, 0x00};
    StrSlice s{bytes, sizeof(bytes)};

    EncodingDetection d = DetectEncoding(s);
    MDVN_CHECK(d.encoding == DetectedEncoding::Utf16Le);
    MDVN_CHECK_EQ(d.contentOffset, 2u);
    ...
    MDVN_CHECK(oddD.encoding == DetectedEncoding::Utf16Le);
    Utf16Slice oddW = DecodeToUtf16(oddS, oddD, &arena);
    MDVN_CHECK_EQ(oddW.len, 2u);
    MDVN_CHECK_EQ(static_cast<int>(oddW.data[0]), 'A');
    MDVN_CHECK_EQ(static_cast<unsigned>(oddW.data[1]), 0xFFFDu);
}
...
MDVN_TEST(Encoding_GbkNoBom_FallsBackToAnsi) {
    MDVN_MAKE_TEST_ARENA();
    ...
    EncodingDetection d = DetectEncoding(s);
    MDVN_CHECK(d.encoding == DetectedEncoding::AnsiFallback);
    MDVN_CHECK_EQ(d.contentOffset, 0u);

    Utf16Slice w = DecodeToUtf16(s, d, &arena);
    MDVN_CHECK(w.data != nullptr);
    MDVN_CHECK(w.len >= 1u);
}

// 样本 5:纯 ASCII,无 BOM,应判定为合法 UTF-8(Utf8NoBom)。
MDVN_TEST(Encoding_PureAscii) {
    MDVN_MAKE_TEST_ARENA();
...
MDVN_TEST(Encoding_IllegalBytes_FallsBackToAnsi) {
    MDVN_MAKE_TEST_ARENA();
