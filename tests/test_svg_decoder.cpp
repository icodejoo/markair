...
MARKAIR_TEST(SvgDecoder_SimpleRectDecodesOk) {
    ImageTestEnv env{};
...
MARKAIR_TEST(SvgDecoder_ClipPathDecodesOk) {
    ImageTestEnv env{};
...
MARKAIR_TEST(SvgDecoder_NullTargetOnlyProbesSize) {
...
MARKAIR_TEST(SvgDecoder_MalformedXmlFailsGracefully) {
...
// 用例:空输入 / 超限输入不崩溃。
MARKAIR_TEST(SvgDecoder_EmptyAndOversizedInputFails) {
    MARKAIR_CHECK(DecodeSvgFromMemory(nullptr, 0, nullptr).status == ImageStatus::Failed);
    char one = 0;
    MARKAIR_CHECK(DecodeSvgFromMemory(&one, 0, nullptr).status == ImageStatus::Failed);
}
