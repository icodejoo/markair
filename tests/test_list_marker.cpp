...
using markair::OrderedListDetail;
...
MARKAIR_TEST(Model_BulletListHasNoOrderedDetail) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] = "- a\n- b\n";
    Document doc = ParseMarkdown(StrSlice{src, sizeof(src) - 1}, &arena);
    MARKAIR_CHECK(!doc.truncated);
    MARKAIR_CHECK_EQ(doc.orderedListDetails.Size(), static_cast<u32>(0));

    u32 ul = FindBlockOfType(doc, BlockType::BulletList, 0);
    MARKAIR_CHECK(ul != kInvalidIndex);
    MARKAIR_CHECK_EQ(doc.blocks[ul].detailIdx, kInvalidIndex);
}

// 用例 2:默认从 1 开始的有序列表,start == 1、分隔符 '.'。
MARKAIR_TEST(Model_OrderedListDefaultStart) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] = "1. a\n2. b\n3. c\n";
    Document doc = ParseMarkdown(StrSlice{src, sizeof(src) - 1}, &arena);
    MARKAIR_CHECK(!doc.truncated);
    ...
    MARKAIR_CHECK(ol != kInvalidIndex);
    MARKAIR_CHECK(doc.blocks[ol].detailIdx != kInvalidIndex);

    const OrderedListDetail& od = doc.orderedListDetails[doc.blocks[ol].detailIdx];
    MARKAIR_CHECK_EQ(od.start, static_cast<u32>(1));
    MARKAIR_CHECK_EQ(static_cast<int>(od.markDelimiter), static_cast<int>('.'));
}

// 用例 3(需求里明确要求的场景):"5. foo" 这种显式指定非 1 起始序号的写法,
// start 必须原样保留成 5,不能被凭空归零/归一。
MARKAIR_TEST(Model_OrderedListExplicitStart) {
    MARKAIR_MAKE_TEST_ARENA();
    ...
    const OrderedListDetail& od = doc.orderedListDetails[doc.blocks[ol].detailIdx];
    MARKAIR_CHECK_EQ(od.start, static_cast<u32>(5));
}

// 用例 4:分隔符为 ')' 的有序列表写法("1) foo")应保留 ')' 而非硬编码成 '.'。
MARKAIR_TEST(Model_OrderedListParenDelimiter) {
    MARKAIR_MAKE_TEST_ARENA();
...
MARKAIR_TEST(Layout_NestedBulletListMarkerLevels) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] =
        "- a\n"
        "  - b\n"
    ...
    MARKAIR_CHECK(li0 != kInvalidIndex);
    MARKAIR_CHECK(li1 != kInvalidIndex);
    MARKAIR_CHECK(li2 != kInvalidIndex);
    ...
    MARKAIR_CHECK_EQ(static_cast<u32>(g0.listMarkerLevel), static_cast<u32>(1));
    MARKAIR_CHECK_EQ(static_cast<u32>(g1.listMarkerLevel), static_cast<u32>(2));
    MARKAIR_CHECK_EQ(static_cast<u32>(g2.listMarkerLevel), static_cast<u32>(3));

    MARKAIR_CHECK(!g0.listMarkerOrdered);
    MARKAIR_CHECK(!g1.listMarkerOrdered);
    MARKAIR_CHECK(!g2.listMarkerOrdered);

    MARKAIR_CHECK(g0.listMarker.width > 0.0f);
    MARKAIR_CHECK(g1.listMarker.width > 0.0f);
    MARKAIR_CHECK(g2.listMarker.width > 0.0f);

    // indent 本身必须保持 M0 既有的"纯嵌套缩进"语义(24/48/72),符号占用的
    // 空间只体现在 listMarkerPad 里——这是本次任务与 M0 基线测试
    // Layout_NestedListIndent 保持兼容的关键设计点。
    MARKAIR_CHECK_EQ(g0.indent, 24.0f);
    MARKAIR_CHECK_EQ(g1.indent, 48.0f);
    MARKAIR_CHECK_EQ(g2.indent, 72.0f);
    MARKAIR_CHECK(g0.listMarkerPad > 0.0f);
}

// 用例 6:有序列表序号从显式起始值递增(对应需求里的"5. foo"验收场景),
// 混入一层嵌套无序列表,确认嵌套不打断有序计数、也不串到无序层级上。
MARKAIR_TEST(Layout_OrderedListOrdinalStartsFromParsedStart) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] =
        "5. foo\n"
        "6. bar\n"
    ...
    MARKAIR_CHECK(li1 != kInvalidIndex);
    MARKAIR_CHECK(li2 != kInvalidIndex);
    MARKAIR_CHECK(li3 != kInvalidIndex);
    ...
    MARKAIR_CHECK(g0.listMarkerOrdered);
    MARKAIR_CHECK(g1.listMarkerOrdered);
    MARKAIR_CHECK(!g2.listMarkerOrdered);  // 嵌套的无序列表项不应该被误标成有序
    MARKAIR_CHECK(g3.listMarkerOrdered);

    MARKAIR_CHECK_EQ(g0.listMarkerOrdinal, static_cast<u32>(5));
    MARKAIR_CHECK_EQ(g1.listMarkerOrdinal, static_cast<u32>(6));
    MARKAIR_CHECK_EQ(g3.listMarkerOrdinal, static_cast<u32>(7));  // 不因为中间插了一层嵌套列表而跳号

    MARKAIR_CHECK_EQ(static_cast<u32>(g2.listMarkerLevel), static_cast<u32>(2));  // 嵌套一层
}

// 用例 7(本次任务重点回归):任务列表项只画勾选框,不应该同时分配列表符号
// 几何——taskCheckbox 与 listMarker 互斥。
MARKAIR_TEST(Layout_TaskListItemDoesNotGetListMarker) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] =
        "- [ ] todo\n"
        "- [x] done\n"
    ...
    MARKAIR_CHECK(g0.listMarker.width <= 0.0f);
    MARKAIR_CHECK_EQ(static_cast<u32>(g0.listMarkerLevel), static_cast<u32>(0));
    MARKAIR_CHECK(g1.taskCheckbox.width > 0.0f);
    MARKAIR_CHECK(g1.listMarker.width <= 0.0f);

    // 普通项:反过来,只画列表符号,不分配勾选框。
    MARKAIR_CHECK(g2.taskCheckbox.width <= 0.0f);
    MARKAIR_CHECK(g2.listMarker.width > 0.0f);
    MARKAIR_CHECK(!g2.listMarkerOrdered);
}
...
MARKAIR_TEST(Layout_MixedNestingBulletThenOrdered) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] =
        "- outer\n"
        "  1. inner-a\n"
    ...
    MARKAIR_CHECK(outer != kInvalidIndex);
    MARKAIR_CHECK(innerA != kInvalidIndex);
    MARKAIR_CHECK(innerB != kInvalidIndex);
    ...
    MARKAIR_CHECK(!gOuter.listMarkerOrdered);
    MARKAIR_CHECK_EQ(static_cast<u32>(gOuter.listMarkerLevel), static_cast<u32>(1));

    MARKAIR_CHECK(gA.listMarkerOrdered);
    MARKAIR_CHECK(gB.listMarkerOrdered);
    MARKAIR_CHECK_EQ(static_cast<u32>(gA.listMarkerLevel), static_cast<u32>(2));
    MARKAIR_CHECK_EQ(static_cast<u32>(gB.listMarkerLevel), static_cast<u32>(2));
    MARKAIR_CHECK_EQ(gA.listMarkerOrdinal, static_cast<u32>(1));
    MARKAIR_CHECK_EQ(gB.listMarkerOrdinal, static_cast<u32>(2));
}
