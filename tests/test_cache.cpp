// T32 覆盖测试:图片缓存(2026-09-17 第二次裁决——解码位图跟随块级虚拟化生灭,
// 尺寸信息常驻;网络图片原始压缩字节去重)。
//
// 覆盖的两条主线:
//   ① 完整生命周期:进入可见范围触发解码 -> 滚出后位图被释放(尺寸仍在)->
//      再次滚入触发重新解码;
//   ② 网络图片原始字节不重复下载(FindRemoteBytes 命中即复用)。
#include "mdvn_test.h"
#include "image_test_support.h"
#include "../src/assets/cache.h"
#include "../src/assets/image.h"
#include "../src/util/arena.h"

#include <cstring>

using mdvn::Arena;
using mdvn::DecodedByteSize;
using mdvn::DecodedImage;
using mdvn::ImageCache;
using mdvn::ImageCacheEntry;
using mdvn::ImageDecoder;
using mdvn::ImageStatus;
using mdvn::StrSlice;
using mdvn::u32;
using mdvn::u8;
using mdvn_test::EncodeTestImage;
using mdvn_test::ImageTestEnv;

namespace {

#define MDVN_MAKE_CACHE_ARENA() Arena arena; arena.Init(4 * 1024 * 1024)

StrSlice Lit(const char* s) { return StrSlice{s, static_cast<u32>(strlen(s))}; }

}  // namespace

// 用例:哈希是纯函数,相同内容的不同缓冲必须得到相同哈希。
MDVN_TEST(Cache_HashIsContentBased) {
    char a[] = "assets/logo.png";
    char b[] = "assets/logo.png";
    MDVN_CHECK(a != b);
    MDVN_CHECK_EQ(mdvn::HashImageKey(Lit(a)), mdvn::HashImageKey(Lit(b)));
    MDVN_CHECK(mdvn::HashImageKey(Lit("a.png")) != mdvn::HashImageKey(Lit("b.png")));
}

// 用例:未命中返回 nullptr;写入后同 key 命中且不重复建条目。
MDVN_TEST(Cache_FindAndPutBasics) {
    MDVN_MAKE_CACHE_ARENA();
    ImageCache cache;
    MDVN_CHECK(cache.Init(&arena));

    MDVN_CHECK(cache.Find(Lit("a.png")) == nullptr);
    MDVN_CHECK_EQ(cache.Count(), 0u);

    cache.Put(Lit("a.png"), nullptr, 100, 50, ImageStatus::Ok, false);
    const ImageCacheEntry* e = cache.Find(Lit("a.png"));
    MDVN_CHECK(e != nullptr);
    if (e) {
        MDVN_CHECK_EQ(e->width, 100u);
        MDVN_CHECK_EQ(e->height, 50u);
        MDVN_CHECK(e->status == ImageStatus::Ok);
    }
    MDVN_CHECK_EQ(cache.Count(), 1u);

    // 同 key 覆盖写不新增条目。
    cache.Put(Lit("a.png"), nullptr, 100, 50, ImageStatus::Ok, true);
    MDVN_CHECK_EQ(cache.Count(), 1u);
}

// 用例:不同图片各自独立计量,大量条目下哈希表扩容后仍然全部可查。
MDVN_TEST(Cache_DistinctKeysAndRehash) {
    MDVN_MAKE_CACHE_ARENA();
    ImageCache cache;
    MDVN_CHECK(cache.Init(&arena));

    // 200 张图,足以触发多次桶扩容(初始 32 桶,负载因子 0.7)。
    char keys[200][16];
    for (u32 i = 0; i < 200; ++i) {
        keys[i][0] = 'i';
        keys[i][1] = 'm';
        keys[i][2] = 'g';
        keys[i][3] = static_cast<char>('0' + (i / 100) % 10);
        keys[i][4] = static_cast<char>('0' + (i / 10) % 10);
        keys[i][5] = static_cast<char>('0' + i % 10);
        keys[i][6] = 0;
        cache.Put(StrSlice{keys[i], 6}, nullptr, 10 + i, 20, ImageStatus::Ok, false);
    }
    MDVN_CHECK_EQ(cache.Count(), 200u);
    for (u32 i = 0; i < 200; ++i) {
        const ImageCacheEntry* e = cache.Find(StrSlice{keys[i], 6});
        MDVN_CHECK(e != nullptr);
        if (e) MDVN_CHECK_EQ(e->width, 10u + i);
    }
}

// 用例(核心):解码位图的完整生命周期 ——
// 进入可见范围解码 -> 计量增加 -> 滚出释放位图(尺寸留存、计量归零)-> 再次滚入重新解码。
MDVN_TEST(Cache_BitmapLifecycleFollowsVisibility) {
    ImageTestEnv env{};
    if (!env.Init()) { env.Shutdown(); return; }

    u8* buf = static_cast<u8*>(HeapAlloc(GetProcessHeap(), 0, 1u << 20));
    if (!buf) { env.Shutdown(); return; }
    u32 n = EncodeTestImage(env.wic, GUID_ContainerFormatPng, 80, 40, 1, buf, 1u << 20);
    MDVN_CHECK(n > 0);

    MDVN_MAKE_CACHE_ARENA();
    ImageCache cache;
    MDVN_CHECK(cache.Init(&arena));
    ImageDecoder decoder;
    StrSlice key = Lit("photo.png");

    // ① 块进入"可见 ± 1 屏":未命中 -> 解码 -> 入缓存。
    MDVN_CHECK(cache.Find(key) == nullptr);
    DecodedImage first = decoder.DecodeFromMemory(buf, n, env.target);
    MDVN_CHECK(first.status == ImageStatus::Ok);
    cache.Put(key, first.bitmap, first.width, first.height, first.status, first.wasDownsampled);

    const ImageCacheEntry* e = cache.Find(key);
    MDVN_CHECK(e != nullptr && e->bitmap != nullptr);
    MDVN_CHECK_EQ(cache.TotalBytes(), DecodedByteSize(80, 40));

    // ② 块滚出可见范围:只丢位图,{width,height} 必须留着(布局不跳动的保证)。
    MDVN_CHECK(cache.ReleaseBitmap(key));
    e = cache.Find(key);
    MDVN_CHECK(e != nullptr);
    if (e) {
        MDVN_CHECK(e->bitmap == nullptr);
        MDVN_CHECK_EQ(e->width, 80u);
        MDVN_CHECK_EQ(e->height, 40u);
    }
    MDVN_CHECK_EQ(cache.TotalBytes(), 0ull);
    MDVN_CHECK_EQ(cache.Count(), 1u);  // 条目本身没被删掉

    // 重复释放是幂等的(返回 false,不重复扣计量)。
    MDVN_CHECK(!cache.ReleaseBitmap(key));
    MDVN_CHECK_EQ(cache.TotalBytes(), 0ull);

    // ③ 块再次滚入可见范围:重新解码,尺寸与第一次完全一致 -> 几何不变,不跳动。
    DecodedImage second = decoder.DecodeFromMemory(buf, n, env.target);
    MDVN_CHECK(second.status == ImageStatus::Ok);
    MDVN_CHECK_EQ(second.width, first.width);
    MDVN_CHECK_EQ(second.height, first.height);
    cache.Put(key, second.bitmap, second.width, second.height, second.status,
              second.wasDownsampled);
    e = cache.Find(key);
    MDVN_CHECK(e != nullptr && e->bitmap != nullptr);
    MDVN_CHECK_EQ(cache.TotalBytes(), DecodedByteSize(80, 40));
    MDVN_CHECK_EQ(cache.Count(), 1u);

    HeapFree(GetProcessHeap(), 0, buf);
    // 必须先把缓存里的位图放掉,再拆 D2D/COM 环境:否则 cache 的析构会在
    // CoUninitialize 之后才 Release,属于典型的 COM 生命周期倒置。
    cache.ReleaseAllBitmaps();
    env.Shutdown();
}

// 用例:解码失败/占位写入不得把已知尺寸清零(否则布局会从真实尺寸退回占位尺寸)。
MDVN_TEST(Cache_PlaceholderPutKeepsKnownSize) {
    MDVN_MAKE_CACHE_ARENA();
    ImageCache cache;
    MDVN_CHECK(cache.Init(&arena));

    cache.Put(Lit("x.png"), nullptr, 300, 200, ImageStatus::Ok, false);
    cache.Put(Lit("x.png"), nullptr, 0, 0, ImageStatus::Failed, false);

    const ImageCacheEntry* e = cache.Find(Lit("x.png"));
    MDVN_CHECK(e != nullptr);
    if (e) {
        MDVN_CHECK_EQ(e->width, 300u);
        MDVN_CHECK_EQ(e->height, 200u);
        MDVN_CHECK(e->status == ImageStatus::Failed);
    }
}

// 用例:ReleaseAllBitmaps(渲染目标重建时调用)只清位图,尺寸与条目数不变。
MDVN_TEST(Cache_ReleaseAllBitmapsKeepsSizes) {
    MDVN_MAKE_CACHE_ARENA();
    ImageCache cache;
    MDVN_CHECK(cache.Init(&arena));
    cache.Put(Lit("a.png"), nullptr, 64, 64, ImageStatus::Ok, false);
    cache.Put(Lit("b.png"), nullptr, 32, 32, ImageStatus::Ok, false);

    cache.ReleaseAllBitmaps();
    MDVN_CHECK_EQ(cache.Count(), 2u);
    MDVN_CHECK_EQ(cache.TotalBytes(), 0ull);
    const ImageCacheEntry* e = cache.Find(Lit("a.png"));
    MDVN_CHECK(e != nullptr);
    if (e) MDVN_CHECK_EQ(e->width, 64u);
}

// 用例(核心):网络图片的原始压缩字节只存一次,后续复用不重复下载。
MDVN_TEST(Cache_RemoteBytesDeduplication) {
    MDVN_MAKE_CACHE_ARENA();
    ImageCache cache;
    MDVN_CHECK(cache.Init(&arena));
    StrSlice url = Lit("https://example.com/badge.svg?x=1");

    // 下载前:查不到 -> 上层据此发起(且仅发起)一次请求。
    u32 len = 0;
    MDVN_CHECK(cache.FindRemoteBytes(url, &len) == nullptr);
    MDVN_CHECK_EQ(len, 0u);

    const char payload[] = "\x89PNG-fake-bytes";
    u32 payloadLen = static_cast<u32>(sizeof(payload) - 1);
    MDVN_CHECK(cache.PutRemoteBytes(url, payload, payloadLen));

    // 下载后:查得到,内容一致,且是缓存自己的副本(不引用调用方缓冲)。
    const u8* got = cache.FindRemoteBytes(url, &len);
    MDVN_CHECK(got != nullptr);
    MDVN_CHECK_EQ(len, payloadLen);
    MDVN_CHECK(got != reinterpret_cast<const u8*>(payload));
    if (got) {
        bool same = true;
        for (u32 i = 0; i < payloadLen; ++i) {
            if (got[i] != static_cast<u8>(payload[i])) same = false;
        }
        MDVN_CHECK(same);
    }

    // 释放解码位图不会连带丢掉已下载的原始字节(压缩字节常驻,避免重复请求)。
    cache.ReleaseBitmap(url);
    MDVN_CHECK(cache.FindRemoteBytes(url, &len) != nullptr);

    // 另一个 URL 仍然查不到,不会被上一条误命中。
    MDVN_CHECK(cache.FindRemoteBytes(Lit("https://example.com/other.png"), &len) == nullptr);
}

// 用例:未 Init 的缓存可以安全调用,不崩溃(退化为永不命中)。
MDVN_TEST(Cache_UninitializedIsSafe) {
    ImageCache cache;
    MDVN_CHECK(!cache.Init(nullptr));
    MDVN_CHECK(cache.Find(Lit("a.png")) == nullptr);
    MDVN_CHECK(cache.Put(Lit("a.png"), nullptr, 1, 1, ImageStatus::Ok, false) == nullptr);
    MDVN_CHECK(!cache.ReleaseBitmap(Lit("a.png")));
    MDVN_CHECK(!cache.PutRemoteBytes(Lit("a.png"), "x", 1));
    MDVN_CHECK_EQ(cache.Count(), 0u);
    MDVN_CHECK_EQ(cache.TotalBytes(), 0ull);
}
