...
MARKAIR_TEST(Layout_CodeBlockHeightDoesNotOverlapNextParagraph) {
    ...
    MARKAIR_CHECK(pGeom.top >= codeGeom.bottom);

    // 代码块本身有 8 个源码行(含两个空行),估算高度应至少覆盖 8 行,
    // 不能因为换行被当成普通字符而被严重低估。
    float lineHeight = 18.0f;  // kMonoLineHeightDip,未缩放
    MARKAIR_CHECK(codeGeom.bottom - codeGeom.top >= lineHeight * 7.0f);
}
...
MARKAIR_TEST(Layout_ParagraphWithSoftBreaksDoesNotOverlapNextBlock) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] =
        "$$\n"
...
MARKAIR_TEST(Layout_MultiLineHtmlBlockReservesEnoughHeight) {
    ...
    MARKAIR_CHECK(htmlIdx != markair::kInvalidIndex);
    ...
    MARKAIR_CHECK(heading.top >= html.bottom);
    // 三行内容(开标签 / 正文 / 闭标签)至少要占 3 行高度。
    float lineHeight = 20.0f;  // kBaseLineHeightDip,未缩放
    MARKAIR_CHECK(html.bottom - html.top >= lineHeight * 3.0f);
}

// 用例 1d(Bug B):行数估算跟"强制换行数"走,而不是跟"总字符数"走——
// 用两个总字符数相近但换行数差异很大的段落对比高度,这是防回归的关键断言
// (与用例 2 对代码块的断言同构,只是把块类型换成 Paragraph)。
MARKAIR_TEST(Layout_ParagraphHeightTracksForcedBreaksNotCharCount) {
    ...
    MARKAIR_CHECK(!docA.truncated);
    MARKAIR_CHECK(!docB.truncated);

    BlockLayoutEngine layoutA, layoutB;
    MARKAIR_CHECK(layoutA.Relayout(docA, 760.0f));
    MARKAIR_CHECK(layoutB.Relayout(docB, 760.0f));

    u32 idxA = FindBlockOfType(docA, BlockType::Paragraph, 0);
    u32 idxB = FindBlockOfType(docB, BlockType::Paragraph, 0);
    MARKAIR_CHECK(idxA != markair::kInvalidIndex);
    MARKAIR_CHECK(idxB != markair::kInvalidIndex);
...
MARKAIR_TEST(Layout_TableCellContentHeightRecordedForCentering) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] =
        "| Short | Long |\n"
        "| --- | --- |\n"
    ...
    u32 shortCellIdx = markair::kInvalidIndex;
    for (u32 i = 0; i < doc.blocks.Size(); ++i) {
    ...
    MARKAIR_CHECK(shortCellIdx != markair::kInvalidIndex);
    ...
    MARKAIR_CHECK(shortCell.textLayout != nullptr);
    // 真实内容高度应该被记录下来(> 0),且明显小于该行的统一行高——
    // 否则渲染层没有依据可以做垂直居中。
    MARKAIR_CHECK(shortCell.contentHeight > 0.0f);
    MARKAIR_CHECK(shortCell.bottom - shortCell.top > shortCell.contentHeight);
}

// 用例 4(T53):未识别语言(kLanguageNone,含无围栏语言标记的代码块)不应该
// 产出任何语法着色 run——不识别的语言一行高亮代码都不跑,直接走原有纯色
// 渲染路径。用一个没有围栏语言标记的缩进代码块验证。
MARKAIR_TEST(Layout_CodeBlockUnknownLanguageProducesNoHighlightRuns) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] =
        "```unknown-lang-xyz\n"
        "def add(a, b):\n"
    ...
    const BlockGeometry& codeGeom = layout.Geometry(codeIdx);
    MARKAIR_CHECK(codeGeom.textLayout != nullptr);
    MARKAIR_CHECK(codeGeom.codeHighlights.len == 0);
}
...
// 地址相同即证明没有发生第二次分配。
MARKAIR_TEST(Layout_CodeHighlightsReusedAcrossVirtualizationCycles) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] =
        "```cpp\n"
    ...
    MARKAIR_CHECK(layout.Geometry(codeIdx).textLayout != nullptr);
    const markair::CodeHighlightRun* firstData = layout.Geometry(codeIdx).codeHighlights.data;
    u32 firstLen = layout.Geometry(codeIdx).codeHighlights.len;
    MARKAIR_CHECK(firstLen > 0);
    MARKAIR_CHECK(firstData != nullptr);
    ...
    for (u32 round = 0; round < 8; ++round) {
        ...
        layout.UpdateVisibleRange(0.0f, 50.0f, fonts);
        MARKAIR_CHECK(layout.Geometry(codeIdx).textLayout != nullptr);
        MARKAIR_CHECK(layout.Geometry(codeIdx).codeHighlights.data == firstData);
        MARKAIR_CHECK(layout.Geometry(codeIdx).codeHighlights.len == firstLen);
    }

    // 重新 Relayout 之后必须重新算(sideDataReady 复位),否则换视口宽度后
    // 沿用的会是按旧宽度算出的偏移。
    MARKAIR_CHECK(layout.Relayout(doc, 400.0f));
    MARKAIR_CHECK(layout.Geometry(codeIdx).codeHighlights.len == 0);
    layout.UpdateVisibleRange(0.0f, 50.0f, fonts);
    MARKAIR_CHECK(layout.Geometry(codeIdx).codeHighlights.len > 0);
}
