...
// 发现:ASAN 构建的 markair.exe(真实 GUI 路径)在本机会在窗口创建前挂死
// (与内存安全无关的构建环境问题,已记录在 bench/M3-LEAK.md 的 T79 章节),
// 因此改用与 test_benchd_smoke.cpp 一致的"直接调用解析/布局函数,不经过窗口"
// 的路子来覆盖 BENCH-A/B/C,与 M1 T43 的既有做法(markair_tests.exe 全量跑)保持
// 一致,不新增第二套语料喂法。
...
MARKAIR_TEST(BenchAbcSmoke_BenchAOpensParsesLayoutsWithoutCrash) {
    RunOpenParseLayoutSmoke(L"BENCH-A.md");
}

// 用例:BENCH-B(图片密集场景基准)。
MARKAIR_TEST(BenchAbcSmoke_BenchBOpensParsesLayoutsWithoutCrash) {
    RunOpenParseLayoutSmoke(L"BENCH-B.md");
}

// 用例:BENCH-C(代码高亮最坏情况基准)。
MARKAIR_TEST(BenchAbcSmoke_BenchCOpensParsesLayoutsWithoutCrash) {
