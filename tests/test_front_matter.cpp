...
MDVN_TEST(FrontMatter_None) {
    const char src[] = "# Title\n\nBody text.\n";
    StrSlice out = SkipFrontMatter(Slice(src));
    MDVN_CHECK_EQ(out.len, static_cast<u32>(sizeof(src) - 1));
    MDVN_CHECK(out.data == src); // 未匹配到 front matter,指针应就是原始起点
}

// 样本 2:正常的 front matter,用 "---" 闭合。
MDVN_TEST(FrontMatter_Normal) {
    const char src[] = "---\ntitle: Hello\n---\n# Body\n";
    StrSlice out = SkipFrontMatter(Slice(src));
    const char expected[] = "# Body\n";
    MDVN_CHECK_EQ(out.len, static_cast<u32>(sizeof(expected) - 1));
    if (out.len == sizeof(expected) - 1) MDVN_CHECK(memcmp(out.data, expected, out.len) == 0);
}

// 样本 2b:用 "..." 闭合同样有效。
MDVN_TEST(FrontMatter_DotsCloser) {
    const char src[] = "---\ntitle: Hello\n...\nBody\n";
    StrSlice out = SkipFrontMatter(Slice(src));
    const char expected[] = "Body\n";
    MDVN_CHECK_EQ(out.len, static_cast<u32>(sizeof(expected) - 1));
...
MDVN_TEST(FrontMatter_Unclosed) {
    const char src[] = "---\ntitle: Hello\nmore: 1\n";
    StrSlice out = SkipFrontMatter(Slice(src));
    MDVN_CHECK_EQ(out.len, static_cast<u32>(sizeof(src) - 1));
    MDVN_CHECK(out.data == src);
}

// 样本 4:首行是 "---" 但其实是分割线——只有一行内容然后 EOF,没有第二个
// "---"/"..." 出现,应原样返回(不误判为 front matter)。
MDVN_TEST(FrontMatter_JustAThematicBreak) {
...
MDVN_TEST(FrontMatter_AfterBomAlreadyStripped) {
