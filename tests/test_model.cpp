...
using markair::kMaxNestingDepth;
...
MARKAIR_TEST(Model_Heading) {
    ...
    MARKAIR_CHECK_STREQ(actual, "H1(-)");
}

// 样本 2:段落。
MARKAIR_TEST(Model_Paragraph) {
    MARKAIR_MAKE_TEST_ARENA();
...
MARKAIR_TEST(Model_BoldItalic) {
    ...
    SummaryBuf buf;
    const char* actual = Summarize(buf, doc);
    MARKAIR_CHECK_STREQ(actual, "P(-,b,-,i,-)");
}

// 样本 4:行内代码。
MARKAIR_TEST(Model_InlineCode) {
    MARKAIR_MAKE_TEST_ARENA();
    ...
    SummaryBuf buf;
    const char* actual = Summarize(buf, doc);
    MARKAIR_CHECK_STREQ(actual, "P(-,c,-)");
}

// 样本 5:有序列表。
MARKAIR_TEST(Model_OrderedList) {
    MARKAIR_MAKE_TEST_ARENA();
    ...
    SummaryBuf buf;
    const char* actual = Summarize(buf, doc);
    MARKAIR_CHECK_STREQ(actual, "OL LI(-) LI(-)");
}

// 样本 6:无序列表。
MARKAIR_TEST(Model_BulletList) {
    MARKAIR_MAKE_TEST_ARENA();
    ...
    SummaryBuf buf;
    const char* actual = Summarize(buf, doc);
    MARKAIR_CHECK_STREQ(actual, "UL LI(-) LI(-)");
}

// 样本 7:引用块。
MARKAIR_TEST(Model_BlockQuote) {
    MARKAIR_MAKE_TEST_ARENA();
    ...
    SummaryBuf buf;
    const char* actual = Summarize(buf, doc);
    MARKAIR_CHECK_STREQ(actual, "BQ P(-)");
}

// 样本 8:分割线。
MARKAIR_TEST(Model_ThematicBreak) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] = "---\n";
    Document doc = ParseMarkdown(StrSlice{src, sizeof(src) - 1}, &arena);
    MARKAIR_CHECK(!doc.truncated);
    SummaryBuf buf;
    const char* actual = Summarize(buf, doc);
    MARKAIR_CHECK_STREQ(actual, "HR");
}

// 样本 9:围栏代码块(无高亮,M0 范围内的最后一类元素)。
MARKAIR_TEST(Model_FencedCodeBlock) {
    MARKAIR_MAKE_TEST_ARENA();
    const char src[] = "```\ncode line\n```\n";
    Document doc = ParseMarkdown(StrSlice{src, sizeof(src) - 1}, &arena);
    MARKAIR_CHECK(!doc.truncated);
...
MARKAIR_TEST(Model_NestingDepthLimit_Truncates) {
    ...
    Arena arena;
    MARKAIR_CHECK(arena.Init(1 * 1024 * 1024));
