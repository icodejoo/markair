...
// 互斥体超时、非法路径),需要真实碰 `%LOCALAPPDATA%\markair\state.ini`。
...
using markair::LoadAppSettings;
using markair::ParseIniSettings;
using markair::SaveAppSettings;
...
// (`%LOCALAPPDATA%\markair\state.ini`),仅供测试直接读写磁盘做断言用。
bool BuildRealStateIniPath(wchar_t* out, size_t cap) {
    ...
    wcscat_s(out, cap, L"\\markair\\state.ini");
    return true;
}
...
struct StateIniBackup {
    ...
    bool hadFile = false;
    char content[markair::kMaxIniBytes]{};
    size_t contentLen = 0;
...
MARKAIR_TEST(Ini_DefaultsAreSafe) {
    AppSettings s;
    DefaultAppSettings(&s);
    MARKAIR_CHECK(s.loadRemoteImages);
    MARKAIR_CHECK_EQ(s.fontBodyPrimary[0], L'\0');
    MARKAIR_CHECK_EQ(s.fontBodyFallback[0], L'\0');
    MARKAIR_CHECK_EQ(s.fontMonoPrimary[0], L'\0');
    MARKAIR_CHECK_EQ(s.fontMonoFallback[0], L'\0');
    // T57:默认缩放档位是 1.0。
    MARKAIR_CHECK(s.zoom > 0.99f && s.zoom < 1.01f);
...
MARKAIR_TEST(Ini_ParsesWindowRectKeys) {
    AppSettings s;
    const char* text =
        "win_x=-100\n"
        "win_y=50\n"
        ...
        "win_h=768\n"
        "win_maximized=1\n";
    MARKAIR_CHECK_EQ(ParseFresh(text, &s), 5u);
    MARKAIR_CHECK_EQ(s.winX, -100);
    MARKAIR_CHECK_EQ(s.winY, 50);
    MARKAIR_CHECK_EQ(s.winW, 1024);
    MARKAIR_CHECK_EQ(s.winH, 768);
    MARKAIR_CHECK(s.winMaximized);
}

// 用例(缺失文件):`%LOCALAPPDATA%\markair\state.ini` 正常情况下不存在,
// LoadAppSettings 必须返回 false 且让配置保持默认值 —— 不创建目录、不写盘。
MARKAIR_TEST(Ini_MissingFileFallsBackToDefaults) {
    AppSettings s;
    ...
    // 这里不对其内容作断言,只要求不崩溃。
    MARKAIR_CHECK(loaded || !loaded);
}

// 用例:基本 KV 解析,开关置 1 生效。
MARKAIR_TEST(Ini_ParsesLoadRemoteImages) {
    AppSettings s;
    MARKAIR_CHECK_EQ(ParseFresh("load_remote_images=1\n", &s), 1u);
    MARKAIR_CHECK(s.loadRemoteImages);

    MARKAIR_CHECK_EQ(ParseFresh("load_remote_images=0\n", &s), 1u);
    MARKAIR_CHECK(!s.loadRemoteImages);
}

// 用例(非法值):无法解析成数字的值不生效,保持默认值,且不算作已应用的键。
MARKAIR_TEST(Ini_InvalidValueKeepsDefault) {
    AppSettings s;
    MARKAIR_CHECK_EQ(ParseFresh("load_remote_images=yes\n", &s), 0u);
    MARKAIR_CHECK(s.loadRemoteImages);

    MARKAIR_CHECK_EQ(ParseFresh("load_remote_images=\n", &s), 0u);
    MARKAIR_CHECK(s.loadRemoteImages);

    MARKAIR_CHECK_EQ(ParseFresh("load_remote_images=1abc\n", &s), 0u);
    MARKAIR_CHECK(s.loadRemoteImages);
}

// 用例(超范围值钳制):超出 [0,1] 的数值被钳制,而不是被当成非法值丢弃。
MARKAIR_TEST(Ini_OutOfRangeValueIsClamped) {
    AppSettings s;
    MARKAIR_CHECK_EQ(ParseFresh("load_remote_images=5\n", &s), 1u);
    MARKAIR_CHECK(s.loadRemoteImages);  // 5 -> 1

    MARKAIR_CHECK_EQ(ParseFresh("load_remote_images=-3\n", &s), 1u);
    MARKAIR_CHECK(!s.loadRemoteImages);  // -3 -> 0

    MARKAIR_CHECK_EQ(ParseFresh("load_remote_images=999999999999\n", &s), 1u);
    MARKAIR_CHECK(s.loadRemoteImages);  // 溢出饱和后仍被钳到 1
}

// 用例(注释行):`;` / `#` 开头的行、`[section]` 行、空行一律跳过。
MARKAIR_TEST(Ini_CommentsAndSectionsSkipped) {
    AppSettings s;
    const char* text =
        "; 这是一行注释\n"
        "# 这也是注释\n"
...
MARKAIR_TEST(Ini_TrimsWhitespaceAndCrlf) {
    AppSettings s;
    MARKAIR_CHECK_EQ(ParseFresh("  load_remote_images  =  1  \r\n", &s), 1u);
    MARKAIR_CHECK(s.loadRemoteImages);
...
MARKAIR_TEST(Ini_FontFamilyOverrides) {
    AppSettings s;
    const char* text =
        ...
        "image_cache_mb=64\n";    // 已作废的键:必须被静默忽略
    MARKAIR_CHECK_EQ(ParseFresh(text, &s), 4u);
    MARKAIR_CHECK(wcscmp(s.fontBodyPrimary, L"Consolas") == 0);
    MARKAIR_CHECK(wcscmp(s.fontBodyFallback, L"微软雅黑") == 0);
    MARKAIR_CHECK(wcscmp(s.fontMonoPrimary, L"Cascadia Code") == 0);
    MARKAIR_CHECK(wcscmp(s.fontMonoFallback, L"Courier New") == 0);
}

// 用例(T57 zoom):档位内的精确值原样保持。
MARKAIR_TEST(Ini_ZoomExactLevelKept) {
    AppSettings s;
    MARKAIR_CHECK_EQ(ParseFresh("zoom=1.3\n", &s), 1u);
    MARKAIR_CHECK(s.zoom > 1.29f && s.zoom < 1.31f);
}

// 用例(T57 zoom):档位外的浮点值被吸附到最近的离散档位,而不是原样采信
// (任意浮点会让 layout 缓存抖动,这正是 T29 选离散档位的理由)。
MARKAIR_TEST(Ini_ZoomSnapsToNearestLevel) {
    AppSettings s;
    MARKAIR_CHECK_EQ(ParseFresh("zoom=1.31\n", &s), 1u);
    MARKAIR_CHECK(s.zoom > 1.29f && s.zoom < 1.31f);  // 吸附到 1.3
}

// 用例(T57 zoom):超出最大档的值被钳到 2.0(最大档),而不是被当成非法值丢弃。
MARKAIR_TEST(Ini_ZoomClampsAboveMax) {
    AppSettings s;
    MARKAIR_CHECK_EQ(ParseFresh("zoom=99\n", &s), 1u);
    MARKAIR_CHECK(s.zoom > 1.99f && s.zoom < 2.01f);
}

// 用例(T57 zoom):非法值(非数字)不生效,回落默认档位 1.0。
MARKAIR_TEST(Ini_ZoomInvalidValueFallsBackToDefault) {
    AppSettings s;
    MARKAIR_CHECK_EQ(ParseFresh("zoom=abc\n", &s), 0u);
    MARKAIR_CHECK(s.zoom > 0.99f && s.zoom < 1.01f);
}

// 用例(T57 zoom):ClampToNearestZoomLevel 与 ParseIniSettings 走同一份档位表,
// 结果必须完全一致(避免两处各存一份容易走样的常量)。
MARKAIR_TEST(Ini_ZoomMatchesFontSubsystemClamp) {
    AppSettings s;
    ParseFresh("zoom=1.31\n", &s);
    MARKAIR_CHECK(s.zoom == markair::FontSubsystem::ClampToNearestZoomLevel(1.31f));
}

// 用例(T57 zoom 写盘 round-trip):写出后再读回,吸附后的值保持不变。
MARKAIR_TEST(Ini_ZoomSaveThenLoadRoundTrips) {
    StateIniBackup backup;
    DeleteFileW(backup.path);

    AppSettings loaded;
    LoadAppSettings(&loaded);  // 建立"磁盘为空"的基线
    loaded.zoom = markair::FontSubsystem::ClampToNearestZoomLevel(1.31f);  // 1.3

    MARKAIR_CHECK(SaveAppSettings(loaded));

    AppSettings readBack;
    MARKAIR_CHECK(LoadAppSettings(&readBack));
    MARKAIR_CHECK(readBack.zoom > 1.29f && readBack.zoom < 1.31f);
}

// 用例:没有 '=' 的行、空键名、超长值都不应导致越界或崩溃。
MARKAIR_TEST(Ini_MalformedLinesAreSafe) {
    AppSettings s;
    MARKAIR_CHECK_EQ(ParseFresh("just some text without equals\n", &s), 0u);
    MARKAIR_CHECK_EQ(ParseFresh("=1\n", &s), 0u);
    ...
    MARKAIR_CHECK(wcslen(s.fontBodyPrimary) < markair::kMaxFontFamilyChars);
    MARKAIR_CHECK(s.fontBodyPrimary[0] == L'X');
}
...
MARKAIR_TEST(Ini_SaveThenLoadRoundTrips) {
    StateIniBackup backup;
    ...
    wcscpy_s(loaded.fontBodyPrimary, markair::kMaxFontFamilyChars, L"Consolas");
    wcscpy_s(loaded.fontMonoFallback, markair::kMaxFontFamilyChars, L"微软雅黑");
    ...
    AppSettings readBack;
    MARKAIR_CHECK(LoadAppSettings(&readBack));
    MARKAIR_CHECK(readBack.loadRemoteImages);
    MARKAIR_CHECK(readBack.theme == ThemeSetting::Dark);
    MARKAIR_CHECK(wcscmp(readBack.fontBodyPrimary, L"Consolas") == 0);
    MARKAIR_CHECK(wcscmp(readBack.fontMonoFallback, L"微软雅黑") == 0);
    MARKAIR_CHECK_EQ(readBack.winX, -200);
    MARKAIR_CHECK_EQ(readBack.winY, 50);
    MARKAIR_CHECK_EQ(readBack.winW, 900);
    MARKAIR_CHECK_EQ(readBack.winH, 650);
    MARKAIR_CHECK(readBack.winMaximized);
}

// 用例(未识别键保留):老版本/手改的键在写盘后必须原样留在文件里。
MARKAIR_TEST(Ini_SaveKeepsUnrecognizedKeys) {
    StateIniBackup backup;
    ...
    HANDLE file = CreateFileW(backup.path, GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS,
                               FILE_ATTRIBUTE_NORMAL, nullptr);
    MARKAIR_CHECK(file != INVALID_HANDLE_VALUE);
    DWORD written = 0;
    ...
    MARKAIR_CHECK(SaveAppSettings(s));

    char content[markair::kMaxIniBytes];
    ReadWholeFile(backup.path, content, sizeof(content));
    MARKAIR_CHECK(strstr(content, "future_flag=42") != nullptr);  // 未知键原样保留
    MARKAIR_CHECK(strstr(content, "load_remote_images=1") != nullptr);  // 已知键被更新
}

// 用例(并发合并):模拟"本进程读盘之后,另一个进程改了别的键",两边改动
// 合并后都要在最终文件里。
MARKAIR_TEST(Ini_SaveMergesConcurrentExternalEdit) {
    StateIniBackup backup;
    DeleteFileW(backup.path);
    ...
    MARKAIR_CHECK(LoadAppSettings(&mergedResult));
    MARKAIR_CHECK(mergedResult.loadRemoteImages);            // "别的进程"的改动保留
    MARKAIR_CHECK(mergedResult.theme == ThemeSetting::Dark);  // 本进程的改动也生效
}
...
DWORD WINAPI HoldMutexThenReleaseThreadProc(LPVOID) {
    HANDLE mutex = CreateMutexW(nullptr, TRUE, markair::kStateIniMutexName);  // 立即持有
    if (mutex) {
        Sleep(markair::kStateIniMutexTimeoutMs + 500);  // 比 SaveAppSettings 的超时更久
        ReleaseMutex(mutex);
        CloseHandle(mutex);
    }
    return 0;
...
MARKAIR_TEST(Ini_SaveGivesUpSilentlyOnMutexTimeout) {
    ...
    MARKAIR_CHECK(GetFileAttributesW(backup.path) == INVALID_FILE_ATTRIBUTES);
    wchar_t tmpPath[MAX_PATH]{};
    wcscpy_s(tmpPath, backup.path);
    wcscat_s(tmpPath, L".tmp");
    MARKAIR_CHECK(GetFileAttributesW(tmpPath) == INVALID_FILE_ATTRIBUTES);
...
// 不崩溃、不抛异常。
MARKAIR_TEST(Ini_SaveHandlesMissingLocalAppDataGracefully) {
