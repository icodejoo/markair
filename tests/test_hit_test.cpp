...
using mdvn::ClientToDocument;
using mdvn::DocPoint;
using mdvn::FindContentBlockAt;
using mdvn::FindImageBoxAt;
using mdvn::HitKind;
...
using mdvn::LinkBox;
using mdvn::LinkTargetAtTextPosition;
using mdvn::ShouldUseHandCursor;
using mdvn::u32;
...
MDVN_TEST(HitTest_ClientToDocumentAppliesScaleAndScroll) {
    DocPoint p = ClientToDocument(200, 100, 2.0f, 300.0f);
    MDVN_CHECK(p.x == 100.0f);
    MDVN_CHECK(p.y == 350.0f);  // 100 / 2 + 300

    // 未缩放、未滚动时是恒等变换。
    DocPoint q = ClientToDocument(40, 60, 1.0f, 0.0f);
    MDVN_CHECK(q.x == 40.0f);
    MDVN_CHECK(q.y == 60.0f);

    // 非法 DPI 缩放退化为 1.0,不产生除零/负坐标。
    DocPoint r = ClientToDocument(10, 20, 0.0f, 5.0f);
    MDVN_CHECK(r.x == 10.0f);
    MDVN_CHECK(r.y == 25.0f);
}

// 用例:容器块不参与命中,内容块参与 —— 这是"块间隙判为没命中"的前提。
MDVN_TEST(HitTest_ContentBlockTypeClassification) {
    MDVN_CHECK(IsContentBlockType(BlockType::Paragraph));
    MDVN_CHECK(IsContentBlockType(BlockType::Heading));
    MDVN_CHECK(IsContentBlockType(BlockType::CodeBlock));
    MDVN_CHECK(IsContentBlockType(BlockType::TableCell));
    MDVN_CHECK(!IsContentBlockType(BlockType::Document));
    MDVN_CHECK(!IsContentBlockType(BlockType::BulletList));
    MDVN_CHECK(!IsContentBlockType(BlockType::BlockQuote));
    MDVN_CHECK(!IsContentBlockType(BlockType::Table));
    MDVN_CHECK(!IsContentBlockType(BlockType::TableRow));
}

// 用例(边界条件:块间隙):两个段落之间留 8 DIP 的空隙,落在空隙里的点
// 不应该命中任何一个段落,而不是"就近吸附"到其中一个。
MDVN_TEST(HitTest_GapBetweenBlocksHitsNothing) {
    ...
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, 10.0f, 10.0f), 1u);
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, 10.0f, 60.0f), 2u);
    // 50 ~ 58 是块间距,属于容器块 —— 容器不参与命中,所以什么都没命中。
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, 10.0f, 54.0f), kInvalidIndex);
    // 半开区间:恰好等于 bottom 的那一行像素算下一个块/间隙,不算上一个块。
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, 10.0f, 50.0f), kInvalidIndex);
    // 文档之外(上方/下方)同样什么都没命中。
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, 10.0f, -1.0f), kInvalidIndex);
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, 10.0f, 999.0f), kInvalidIndex);
}

// 用例:嵌套时取最深的那个块(前序数组里下标更大者)。
MDVN_TEST(HitTest_NestedBlocksPickDeepest) {
    BlockGeometry geoms[3] = {
    ...
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, 5.0f, 20.0f), 2u);
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, 5.0f, 60.0f), kInvalidIndex);
}

// 用例:同一行的表格单元格 y 区间完全相同,必须按 x 与列宽区分,不能互相误命中。
MDVN_TEST(HitTest_TableCellsDistinguishedByColumn) {
    BlockGeometry geoms[3] = {
    ...
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, 50.0f, 20.0f), 1u);
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, 180.0f, 20.0f), 2u);
    // 落在两列之间的单元格内边距/网格线上:不属于任何一个单元格。
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, 115.0f, 20.0f), kInvalidIndex);
}

// 用例(边界条件:滚动后的坐标):同一个客户区像素点,在不同滚动偏移下
// 命中的块必须跟着变。
MDVN_TEST(HitTest_ScrolledCoordinatesHitDifferentBlocks) {
    BlockGeometry geoms[3] = {
    ...
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, top.x, top.y), 1u);

    // 向下滚 500 DIP 之后,同一个客户区坐标落到了第二个段落上。
    DocPoint scrolled = ClientToDocument(10, 50, 1.0f, 500.0f);
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, scrolled.x, scrolled.y), 2u);

    // 滚到两块之间的空白处:仍然什么都没命中。
    DocPoint between = ClientToDocument(10, 50, 1.0f, 200.0f);
    MDVN_CHECK_EQ(FindContentBlockAt(geoms, 3, between.x, between.y), kInvalidIndex);
}

// 用例:图片矩形命中,含左闭右开的边界语义。
MDVN_TEST(HitTest_ImageBoxRectangles) {
    ImageBox boxes[2]{};
    ...
    MDVN_CHECK_EQ(FindImageBoxAt(boxes, 2, 50.0f, 50.0f), 0u);
    MDVN_CHECK_EQ(FindImageBoxAt(boxes, 2, 50.0f, 150.0f), 1u);
    MDVN_CHECK_EQ(FindImageBoxAt(boxes, 2, 10.0f, 20.0f), 0u);    // 左上角在内
    MDVN_CHECK_EQ(FindImageBoxAt(boxes, 2, 110.0f, 50.0f), kInvalidIndex);  // 右边界在外
    MDVN_CHECK_EQ(FindImageBoxAt(boxes, 2, 50.0f, 105.0f), kInvalidIndex);  // 两张图之间
    MDVN_CHECK_EQ(FindImageBoxAt(nullptr, 0, 0.0f, 0.0f), kInvalidIndex);
}

// 用例(边界条件:行尾空白):链接 run 是半开区间,正好落在 run 末尾之后的位置
// (点在链接文字右侧的行尾空白上)不应该判为命中链接。
MDVN_TEST(HitTest_LinkRunIsHalfOpenRange) {
    LinkBox boxes[2] = {
        LinkBox{0u, 5u, 4u, 11u},   // [5, 9)
        LinkBox{0u, 20u, 3u, 22u},  // [20, 23)
    };

    MDVN_CHECK_EQ(LinkTargetAtTextPosition(boxes, 2, 5u), 11u);
    MDVN_CHECK_EQ(LinkTargetAtTextPosition(boxes, 2, 8u), 11u);
    MDVN_CHECK_EQ(LinkTargetAtTextPosition(boxes, 2, 9u), kInvalidIndex);  // 行尾空白
    MDVN_CHECK_EQ(LinkTargetAtTextPosition(boxes, 2, 4u), kInvalidIndex);
...
MDVN_TEST(HitTest_HandCursorOnlyForInteractive) {
    ...
    MDVN_CHECK(!ShouldUseHandCursor(none));
    MDVN_CHECK(ShouldUseHandCursor(link));
    MDVN_CHECK(ShouldUseHandCursor(image));
}
