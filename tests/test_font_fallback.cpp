...
MDVN_TEST(FontFallback_MonoChainContainsCjkFamilies) {
    u32 count = 0;
    const wchar_t* const* chain = FontSubsystem::MonoFallbackFamilies(&count);
    MDVN_CHECK(chain != nullptr);
    MDVN_CHECK(count >= 5u);

    MDVN_CHECK(ChainContains(chain, count, L"Consolas"));
    MDVN_CHECK(ChainContains(chain, count, L"Courier New"));
    MDVN_CHECK(ChainContains(chain, count, L"Microsoft YaHei UI"));
    MDVN_CHECK(ChainContains(chain, count, L"Microsoft YaHei"));
    MDVN_CHECK(ChainContains(chain, count, L"SimSun"));
}

// 用例 2:优先级必须是"西文等宽在前、中文兜底在后"——否则西文代码会被
// 中文字体的非等宽字形接走,代码块失去等宽对齐。
MDVN_TEST(FontFallback_MonoChainPrefersWesternMonoBeforeCjk) {
    u32 count = 0;
    const wchar_t* const* chain = FontSubsystem::MonoFallbackFamilies(&count);
    MDVN_CHECK(chain != nullptr);
    ...
    MDVN_CHECK(consolas < courier);
    MDVN_CHECK(courier < yahei);
    MDVN_CHECK(yahei < simsun);
}

// 用例 3:等宽主族仍是裁决 #10 的 Cascadia Mono,追加中文兜底不改主族。
MDVN_TEST(FontFallback_MonoPrimaryFamilyUnchanged) {
    MDVN_CHECK(WEquals(FontSubsystem::MonoFamilyName(), L"Cascadia Mono"));
}
